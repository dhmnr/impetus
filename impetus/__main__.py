...
from .models import DecoderOnlyLanguageModel
...
@cli.command()
@click.option(
    "--model", type=click.STRING, required=True, help="LLM model to benchmark."
)
@click.option(
    "--device",
...
@click.option("--sequence-length", type=int, default=128, help="Input sequence length.")
@click.option(
    "--precision",
    type=click.Choice(["4bit", "8bit", "full"]),
    default="full",
    help="Precison to use for the models",
)
@click.option(
    "--num-runs", type=int, default=10, help="Number of inference iterations."
)
...
def benchmark(
    ...
    click.echo(f"Batch size: {batch_size}, Sequence length: {sequence_length}")
    click.echo(f"Running {num_runs} iterations with {warmup_runs} warmup iterations")

    lm = DecoderOnlyLanguageModel(model, device=device, precision=precision)
    metrics = lm.benchmark_inference(
